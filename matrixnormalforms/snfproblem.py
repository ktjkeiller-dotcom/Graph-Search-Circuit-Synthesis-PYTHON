...
class SNFProblem:
    ...
    def is_valid(self):
        ...
        if not self.S.determinant().is_unit():
            print("S not modular")
            return False

        # check that T is unimodular
        elif not self.T.determinant().is_unit():
            print("T not modular")
            return False
        ...
        if diagLength > 1:
            ...
            for i in range(1, diagLength):
                ...
                #print("Checking that x¦y ")
                #print("last diag norm: ",lastDiag.norm())
                #print("current diag norm: ",currentDiag.norm())
                #print("lastDiag divides currentDiag: ", (currentDiag % lastDiag ==zero))
    ...
    # continues to hold.
    def c_lc(self, k, i, j, a, b, gcd=None):
        if self.debug:
            print("c_lc call")
    ...
    def compute_snf(self):
        ...
        for i in range(min(self.J.h, self.J.w)):
            ...
            if self.J.get(i, i) == self.elementT.get_zero():
                ...
                for j in range(i, self.J.h):
                    ...
                    for k in range(i, self.J.w):
                        if self.J.get(j, k) != self.elementT.get_zero():
            ...
            while not doneIteration:
                if self.J.get(i, i).is_unit():
                    break
                doneIteration = True
                for j in range(i + 1, self.J.h):
                    gcd, x, y = self.J.get(i, i).extended_gcd(self.J.get(j, i))
                    if self.J.get(i, i).is_unit_multiple_of(gcd):
                        pass
                    elif self.J.get(j, i).is_unit_multiple_of(gcd):
                        self.r_swap(i, j)
                        doneIteration = False
                    else:
                        self.r_lc(i, i, j, x, y, gcd)
                        doneIteration = False
                for j in range(i + 1, self.J.w):
                    gcd, x, y = self.J.get(i, i).extended_gcd(self.J.get(i, j))
                    if self.J.get(i, i).is_unit_multiple_of(gcd):
                        pass
                    elif self.J.get(i, j).is_unit_multiple_of(gcd):
                        self.c_swap(i, j)
                        doneIteration = False
                    else:
                        self.c_lc(i, i, j, x, y, gcd)
                        doneIteration = False
            ...
            while not doneZeroing:
                doneZeroing = True
                for j in range(i + 1, self.J.h):
                    if self.J.get(j, i) != self.elementT.get_zero():
                        self.r_lc(i, j, i, self.elementT.get_one(),
                                 -self.J.get(j, i) // self.J.get(i, i))
                        if self.J.get(j, i) != self.elementT.get_zero():
                            doneZeroing = False
                for j in range(i + 1, self.J.w):
                    if self.J.get(i, j) != self.elementT.get_zero():
                        self.c_lc(i, j, i, self.elementT.get_one(),
        ...
        for j in range(min(self.J.w, self.J.h) - 1):
            # If the next diagonal element is 0, then all following diagonal
            # elements # will be 0. Therefore self.J is in Smith normal form
            # and we return
            for i in range(min(self.J.w, self.J.h)-j-1):
                if self.J.get(i + 1, i + 1) == self.elementT.get_zero():
                    return
                ...
                #it might be that the gcd is the element multiplied by a unit 
                
                if (self.J.get(i+1,i+1).is_equivalent(gcd)):
                    #if gcd == self.J.get(i + 1, i + 1):
                    self.c_swap(i, i + 1)
                    self.r_swap(i, i + 1)

                #elif gcd != self.J.get(i, i):
                elif not(self.J.get(i,i).is_equivalent(gcd)):
                    swapped = True
                    self.r_lc(i, i, i + 1, self.elementT.get_one(),
                            self.elementT.get_one())
                    self.c_lc(i, i, i + 1, x, y, gcd)
                    self.c_lc(i, i + 1, i, self.elementT.get_one(),
                            -self.J.get(i, i + 1) // self.J.get(i, i))
                    self.r_lc(i, i + 1, i, self.elementT.get_one(),
